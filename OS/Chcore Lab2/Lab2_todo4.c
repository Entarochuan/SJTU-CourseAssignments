                        ...
                        pte_t **pte, bool alloc)
{
        ...
        if (IS_PTE_INVALID(entry->pte)) {
                if (alloc == false) {
                        ...
                        new_pte_val.pte = 0;
                        new_pte_val.table.is_valid = 1;  // add information of current page
...
int query_in_pgtbl(void *pgtbl, vaddr_t va, paddr_t *pa, pte_t **entry)
{
	...
	pte_t *next_pte ;
        ...
        for (int level = 0 ; level<=3; level++){
                err = get_next_ptp(cur_ptp, level, va, &cur_ptp, &next_pte, false) ;
                if (err == -ENOMAPPING) return -ENOMAPPING ;
                
                else if ( level == 1 && err == BLOCK_PTP) {
                        if (entry != NULL) {
                                *entry = next_pte ;
                        }
...
int unmap_range_in_pgtbl(void *pgtbl, vaddr_t va, size_t len)
{
        ...
        while (page_num > 0) {
                ...
                for(int level = 0 ; level<=2; level++){
                        err = get_next_ptp(cur_ptp, level, va, &cur_ptp, &pte, false);

                        if (err == -ENOMAPPING && level==0) {
